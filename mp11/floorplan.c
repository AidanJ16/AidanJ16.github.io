...
// INTRO PARAGRAPH
// In this program we had to complete the following functions:
// The first is init_slicing_tree, where we had to generate a slicing tree that results
// in a horizontally aligned floorplan. We had to implement this function recursively.
// The general procedure for the function was to generate a internal node with vertical
// cutline, a right child attached by module pointer, and then a left child with which we would step to
// the next recursion parented by the current internal node.
// The next function is get_total_resource where we traversed a tree and returned the total
// sum of resource of the modules.
// The next function is is_leaf_node where we checked a node and returned if it was a leaf
// The next function is is_internal_node where we checked a node and returned if it was a internal node
// The next function is is_in_subtree that compares whether subtree rooted at b belongs to part of
// subtree rooted at a.
// The next function is get_expression that traverses the tree in postfix order to encode it
// into a postfix expression
// The next function is postfix_traversal, an internal call of the previous function,
// that helpes perform the postfix traversal recursively
// The next function is rotate that takes a node and roates the module 90 degrees
// The next function is recut that changes the cutline from horizontal to vertical or vice versa
// The next function swap_module swaps the module pointers of two nodes
// The next function swap_topology swaps two subtrees
//
...
int is_leaf_node(node_t* ptr) {
  ...
  // if node has no left or right child it is leaf node
...
int is_internal_node(node_t* ptr) {
  // TODO: (remember to modify the return value appropriately)
  // if node has left or right child it is an internal node
  if(ptr->right != NULL || ptr->left != NULL) {
    return 1;
  }
  // else not an internal node
  return 0;
}
...
int is_in_subtree(node_t* a, node_t* b) {
  // TODO: (remember to modify the return value appropriately)
  // if a is null b cannot be in a
  if(a == NULL) {
    return 0;
  }
  // if they are the same nodes then b does reside in subtree at a
  if(a == b) {
    return 1;
  }
  // check a's left child 
  if(is_in_subtree(a->left, b) == 1) {
    return 1;
  }
  // check a's right child
  if(is_in_subtree(a->right, b) == 1) {
    return 1;
  }
  return 0;
}
...
void rotate(node_t* ptr) {
  // TODO:
  // swap the node's w and h using a tmp variable
  int tmp = ptr->module->w;
...
void recut(node_t* ptr) {
  ...
  // TODO:
  // if cutline is V change to H
  // else cutline is H so change to V
  if(ptr->cutline == V) {
    ptr->cutline = H;
  } else {
    ptr->cutline = V;
...
void swap_module(node_t* a, node_t* b) {
  ...
  // TODO:
  // swap the modules of a and b using tmp variable
...
void swap_topology(node_t* a, node_t* b) {
  ...
  // create tmp to save a's parent
  node_t *tmp = a->parent;
  // if a is right child of it's parent, change right pointer of parent to b
  // else a is left child so change left pointer of parent to b
  if(a->parent->right == a) {
    a->parent->right = b;
  } else {
    a->parent->left = b;
  }
  // if b is right child of it's parent, change right pointer of parent to a
  // else b is left child so change left pointer of parent to a
  if(b->parent->right == b) {
    b->parent->right = a;
  } else {
    b->parent->left = a;
  }
  // finally we swap parent pointers using tmp variable
  a->parent = b->parent;
  b->parent = tmp;
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression) {
  ...
  if(ptr == NULL) return;
  // recursive call the left and right child to traverse the tree
  postfix_traversal(ptr->left, nth, expression);
  postfix_traversal(ptr->right, nth, expression);

  // if module is not null we write the node's module to our array
  // cutline must be UNDEFINED_CUTLINE
  if(ptr->module != NULL) {
    expression[*nth].cutline = UNDEFINED_CUTLINE;
    expression[*nth].module = ptr->module;
  }
  // else module is null so fill appropriately
  // cutline must be node's cutline
  else {
    expression[*nth].cutline = ptr->cutline;
    expression[*nth].module = NULL;
  }
  // increment position in array after filled
  (*nth) = 1 + (*nth);
  return;
}
...
int get_total_resource(node_t* ptr)
{
  ...
  // set up sum var
  int sum = 0;
  // ensure node isn't null, then check if it is a leaf node
  // if so increment sum accordingly
  if(ptr != NULL && is_leaf_node(ptr)) {
    sum += ptr->module->resource;
  }
  // ensure node isn't null, must be internal node so recursively call on the left and right child and update sum
  if(ptr != NULL) {
    sum += get_total_resource(ptr->left);
    sum += get_total_resource(ptr->right);
  }
  // when traversed whole tree return sum
  return sum;
}
...
node_t* init_slicing_tree(node_t* par, int n) {
  ...
  // create a new node that is child of given node
  node_t* curr = (node_t*) malloc(sizeof(node_t));
  curr->parent = par;

  // TODO:
  // base case, if index is 1 less than number of modules
  // node is leaf, so update modules based on n, cutline to UNDEFINED_CUTLINE
  // left and right child to null
  if(n == num_modules - 1) {
    curr->left = NULL;
    curr->right = NULL;
    curr->module = modules + n;
    curr->cutline = UNDEFINED_CUTLINE;
    return curr;
  }
  // otherwise node is internal, so create right child as a leaf node
  // update modules based on n, cutline to UNDEFINED_CUTLINE
  // left and right child to null
  ...
  // current node is internal so defined cutline as V, module as NULL, and connect right child
  curr->cutline = V;
  curr->module = NULL;
  curr->right = r_child;
  // recursively call to fill in left child of our current node
  curr->left = init_slicing_tree(par, n+1);
  
  // whole tree is filled so return
  return curr;
}
