...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
     ...
     */
     ...
     while(!singleQueue.empty() || mergeQueue.size() != 1) {
        TreeNode * tmp1 = removeSmallest(singleQueue, mergeQueue);
        TreeNode * tmp2 = removeSmallest(singleQueue, mergeQueue);
        /*if(singleQueue.empty()) {
            tmp1 = mergeQueue.front();
            mergeQueue.pop();
            tmp2 = mergeQueue.front();
            mergeQueue.pop();
        } else if(mergeQueue.empty()) {
            tmp1 = singleQueue.front();
            singleQueue.pop();
            tmp2 = singleQueue.front();
            singleQueue.pop();
        } else {
            if(singleQueue.front()->freq.getFrequency() <= mergeQueue.front()->freq.getFrequency()) {
                tmp1 = singleQueue.front();
        ...
        int new_freq = tmp1->freq.getFrequency() + tmp2->freq.getFrequency();
        TreeNode * curr = new TreeNode(new_freq);
        curr->left = tmp1;
        curr->right = tmp2;
