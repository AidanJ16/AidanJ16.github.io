...
// MP2 part 1: load preferences from CSV and convert to JSON
fun loadPreferences(): String {
    val input = Server::class.java.getResource("/preferences.csv")!!.readText()
    val arrayNodeReturn = JsonNodeFactory.instance.arrayNode()
    for (line in input.lines()) {
        ...
        val arrayNode = JsonNodeFactory.instance.objectNode().apply {
            put("id", parts[0])
            put("restaurantIDs", node)
        }
        arrayNodeReturn.add(arrayNode)
    }
    return arrayNodeReturn.toPrettyString()
}
