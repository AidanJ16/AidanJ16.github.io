...
class KDTree
{
  ...
  public:
    ...
    void build_KDTree(vector<Point<Dim>>& vect, KDTreeNode*& node, int dim, int start, int end);
    ...
    void copy_tree(KDTreeNode*& root, KDTreeNode*& o_root);
    /**
    ...
    void delete_tree(KDTreeNode*& to_del);
    /**
    ...
    Point<Dim> findNearest(KDTreeNode * node, Point<Dim>& query, int dim) const;

    int findDistance(Point<Dim>& one, Point<Dim>& two) const;
