...
void BTree<K, V>::split_child(BTreeNode* parent, size_t child_idx)
{
    ...
    /* Iterator for where we want to insert the new child. (new_right child) */
    auto child_itr = parent->children.begin() + child_idx + 1;
    /* Iterator for where we want to insert the new element. (parent_element vector where i insert child) */
    auto elem_itr = parent->elements.begin() + child_idx;
    /* Iterator for the middle element.  middle element of the child elements vector*/
    ...
    //"Insert" new median child value to parent elements 
    parent->elements.insert(elem_itr, *mid_elem_itr);

    //Go to right of median element in new_left elements, and copy over all elements/children past the median index, and erase them from their current vectors
    for (size_t t = mid_elem_idx + 1; t < child->elements.size(); ++t) {
        new_right->elements.push_back(child->elements.at(t));
    }
    new_left->elements.erase(mid_elem_itr, new_left->elements.end());
    if(child->is_leaf != true) {
        for (size_t t = mid_child_idx; t < child->children.size(); ++t) {
            new_right->children.push_back(child->children.at(t));
        }
        new_left->children.erase(mid_child_itr, child->children.end());
    }

    //Remove the median value from the new_left child
    /*auto torem = --(new_left->elements.end());
    new_left->elements.erase(torem);*/

    //Insert new left and right at valid indices inside of parent's children
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    /* TODO Your code goes here! */
    if(subroot->is_leaf) {
        if((subroot->elements.size() == 0) || (first_larger_idx == subroot->elements.size())) {
            subroot->elements.push_back(pair);
        } else if(first_larger_idx < subroot->elements.size() && !(subroot->elements.at(first_larger_idx) == pair)) {
            subroot->elements.insert(subroot->elements.begin() + first_larger_idx, pair);
        }
    } else {
        BTreeNode * child = subroot->children.at(first_larger_idx);
        insert(child, pair);
